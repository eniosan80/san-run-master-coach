...
export type Phase = "Adaptação" | "Construção" | "Evolução" | "Desempenho" | "Retorno";
...
export interface WorkoutBlock {
  ...
  label: string;
  durationMin?: number;    // warmup / cooldown / continuous
  rpe?: number;
  // series-specific
  reps?: number;
...
export function classifyAthlete(input: AthleteInput): Classification {
  let score = 0;

  // Experience score
  if (input.experience === "never") score += 0;
  else if (input.experience === "beginner") score += 1;
  else if (input.experience === "intermediate") score += 2;
  else if (input.experience === "advanced") score += 3;

  // Frequency score
  if (input.weeklyFrequency === "0") score += 0;
  else if (input.weeklyFrequency === "1-2") score += 1;
  else if (input.weeklyFrequency === "3-4") score += 2;
  else if (input.weeklyFrequency === "5+") score += 3;

  // Age modifier (small impact)
  if (input.age < 20) score += 0;
  else if (input.age <= 40) score += 0;
  ...
  const nextFocuses: Record<number, string> = {
  ...
  return {
    ...
    nextFocus: nextFocuses[level],
    phase: phases[level],
  };
}

export function generateDiagnosis(input: AthleteInput, classification: Classification): Diagnosis {
  ...
  return {
    data: `${input.age} anos, ${input.sex === "male" || input.sex === "M" ? "masculino" : input.sex === "female" || input.sex === "F" ? "feminino" : "outro"}, ${expLabels[input.experience]}, com ${freqLabels[input.weeklyFrequency]}. Objetivo declarado: "${input.goal}".`,
    interpretation: `O atleta está no Nível ${classification.level} — ${classification.levelName}. ${classification.reason} A fase atual é ${classification.phase}, o que indica que o foco principal deve ser em ${classification.nextFocus.toLowerCase()}.`,
    decision: `Iniciar protocolo de ${classification.phase}. ${classification.nextFocus} O objetivo declarado será usado como direção — mas o caminho começa pelo processo, não pelo destino.`,
  };
}

/** Biblioteca completa de treinos SAN RUN com blocos estruturados */
export const WORKOUT_LIBRARY: Record<number, WorkoutRecommendation[]> = {
  1: [
    {
      ...
      why: "Seu corpo está se adaptando ao impacto da corrida. Alternar caminhada e trote constrói base sem sobrecarregar articulações e músculos.",
      successCriteria: "Manter o trote sem parar antes do tempo e chegar ao final sem dor ou desconforto excessivo.",
      blocks: [
        { type: "warmup", label: "Aquecimento", durationMin: 10, rpe: 2 },
        { type: "series", label: "Trote Leve", reps: 4, workDurationMin: 1, restDurationMin: 2, workRpe: 3, restRpe: 2, restLabel: "Caminhada LE" },
    ...
    },
    {
      ...
      duration: "30 min",
      rpe: 2,
      instructions: "Caminhe em ritmo constante e confortável. Mantenha postura ereta, ombros relaxados. Respire pelo nariz quando possível. Você deve conseguir conversar sem dificuldade.",
      why: "Ativar o sistema cardiovascular de forma gentil. Este treino cria o hábito e prepara o corpo para estímulos maiores.",
      successCriteria: "Terminar o tempo proposto sem exaustão. Sentir que poderia continuar por mais tempo.",
      blocks: [
    ...
    },
    {
      ...
      why: "Introduzir o trote de forma gradual, respeitando o ritmo do seu corpo.",
  ...
  2: [
    {
      ...
      duration: "30 min",
      rpe: 4,
      instructions: "Corra em ritmo leve e constante. O teste: você deve conseguir falar frases curtas enquanto corre. Se não conseguir, reduza o ritmo. Não precisa ser rápido — precisa ser contínuo.",
      why: "Desenvolver resistência aeróbica base é o alicerce de qualquer progresso. Correr devagar hoje é o que permite correr mais rápido no futuro.",
      successCriteria: "Completar o tempo sem pausas. Ritmo cardíaco controlado. Terminar cansado, mas não esgotado.",
      blocks: [
    ...
    },
    {
      ...
      duration: "35 min",
      rpe: 6,
      instructions: "5 min aquecimento. 8 tiros de: 30 segundos em ritmo forte + 1 min caminhada. 10 min corrida leve. 5 min desaquecimento.",
      why: "Introduzir estímulos de velocidade curtos para desenvolver potência sem acumular fadiga excessiva.",
    ...
    },
    {
      ...
      why: "Aprender a progredir o esforço ao longo do treino — habilidade fundamental para provas.",
  ...
  3: [
    ...
    },
    {
      ...
      successCriteria: "Manter o mesmo ritmo nas 5 acelerações. Recuperação completa nos intervalos.",
      blocks: [
        { type: "warmup", label: "Aquecimento", durationMin: 10, rpe: 4 },
    ...
    },
    {
      ...
      why: "Volume aeróbico é o que constrói a base para tudo. O longo semanal é insubstituível.",
  ...
  4: [
    {
      ...
      duration: "45 min",
      rpe: 7,
      instructions: "Divida o treino em 3 partes iguais: primeiro terço em ritmo leve, segundo terço em ritmo moderado, último terço em ritmo forte (mas sustentável). Não saia rápido — termine forte.",
      why: "Treinos progressivos desenvolvem controle de esforço e eficiência metabólica. Aprender a acelerar no final é uma habilidade competitiva.",
    ...
    },
    {
      ...
      why: "Intervalados de alta intensidade aumentam VO2max e velocidade de corrida. Treino de alta qualidade.",
    ...
    },
    {
      ...
      why: "Longão progressivo constrói resistência e ensina o corpo a usar energia de forma eficiente.",
  ...
  5: [
    {
      ...
      why: "Intervalados de alta intensidade aumentam VO2max, velocidade limiar e capacidade de repetir esforços. Treino fundamental para performance.",
      successCriteria: "Manter o mesmo ritmo do bloco 1 no bloco 6. Frequência cardíaca que responde e se recupera.",
      blocks: [
    ...
    },
    {
      ...
      why: "Treino de limiar aumenta a velocidade que você consegue manter por longos períodos — base da performance.",
...
/** Treino de recuperação (readiness baixa) */
const RECOVERY_WORKOUT: WorkoutRecommendation = {
  title: "Recuperação Ativa",
  duration: "20 min",
  rpe: 2,
  instructions: "Caminhada leve ou trote muito suave por 15-20 minutos. O objetivo é mover o corpo sem criar estresse adicional.",
  why: "Recuperação é treino. Respeitar os sinais do corpo é o que separa atletas que evoluem dos que se machucam.",
  successCriteria: "Sentir-se melhor ao final do que no início. Zero dor nova.",
  blocks: [
    { type: "continuous", label: "Caminhada LE", durationMin: 20, rpe: 2 },
  ],
};

export function generateWorkout(level: number, phase: Phase, checkinReadiness?: string): WorkoutRecommendation {
  const readiness = checkinReadiness || "high";
  const lib = WORKOUT_LIBRARY[Math.min(Math.max(level, 1), 5)];
  let recommendation = lib[0];

  if (readiness === "low") {
    return { ...RECOVERY_WORKOUT, title: "Recuperação Ativa — " + recommendation.title };
  }
  if (readiness === "medium") {
    return { ...recommendation, rpe: Math.max(1, recommendation.rpe - 1), title: "Treino Ajustado — " + recommendation.title };
  }
