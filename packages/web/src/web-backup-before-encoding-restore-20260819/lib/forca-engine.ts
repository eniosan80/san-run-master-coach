...
function prescribeBlocks(profile: ForcaProfile): ForcaBlock[] {
  ...
  /* =========================================================
     2. ATIVA��O � exatamente 2
     Prioriza as necessidades apontadas na avalia��o.
     ========================================================= */
  ...
  /* =========================================================
     3. FOR�A � m�ximo 3
     Prioriza a �rea fraca do corredor.
     ========================================================= */
  ...
  /* Seguran�a: nunca ultrapassar 7 exerc�cios */
...
function planTitle(profile: ForcaProfile): string {
  ...
  if (hasLombar) return "Prote��o Lombar + Core";
  if (hasJoelho) return "Estabilidade de Joelho";
  if (profile.weakAreas.includes("pernas") && profile.weakAreas.includes("core")) return "For�a Funcional";
  if (profile.weakAreas.includes("gluteos")) return "Ativa��o de Gl�teos";
...
function planObjective(profile: ForcaProfile): string {
  ...
  if (profile.discomfort.includes("joelho")) return "Fortalecer estabilizadores do joelho e reduzir risco de les�o.";
  if (profile.discomfort.includes("lombar")) return "Ativar core profundo e proteger a lombar durante as passadas.";
  return "Desenvolver for�a espec�fica para corrida, melhorar economia de movimento e prevenir les�es.";
}

/* ---------------------------------------------
   FUN��O PRINCIPAL
   trainingDays: dias JS de corrida (0=Dom�6=S�b)
   Retorna ForcaPlan com focusDays = dias sem corrida (at� daysPerWeek)
--------------------------------------------- */
export function generateForcaPlan(profile: ForcaProfile, trainingDays: number[]): ForcaPlan {
  ...
  // Preferir dias imediatamente ap�s corrida (recovery ativo)
  // Se n�o h� dias de descanso suficientes, pegar dias de treino com menor conflito
  let focusDays: number[];
  if (restDays.length >= profile.daysPerWeek) {
    // Ordenar: preferir dias logo ap�s um dia de treino
    ...
    // Pegar todos os dias de descanso dispon�veis
