...
function blockLabel(type: BlockType): string {
  switch (type) {
    ...
    case "series":   return "Série / Bloco";
      ...
      <div style={{
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <input
            ...
            placeholder="Ex: Corrida contínua"
            style={{
        ...
        {e.type === "series" ? (
          <>
            <NumberStepper label="Repetições" value={e.reps} onChange={v => set({ reps: v })} min={1} max={50} />
            <NumberStepper
            ...
            />
            <NumberStepper
              label="Recuperação entre séries (min)"
            ...
            <RpeRow label="RPE — Esforço" value={e.workRpe} onChange={v => set({ workRpe: v })} />
          </>
        ) : (
          <>
            <NumberStepper label="Duração (min)" value={e.durationMin} onChange={v => set({ durationMin: v })} min={1} max={120} />
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          <textarea
            ...
            rows={2}
            placeholder="Ritmo, percepção, foco..."
            style={{
              width: "100%", background: C.input, border: `1px solid ${C.border}`,
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, color: C.offwhite, paddingBottom: 120 }}>
      ...
      <div style={{ display: "flex", gap: 8, padding: "18px 20px 0" }}>
        {[
          { label: "DURAÇÃO", value: formatDurationMin(total), color: C.terracota },
          { label: "ETAPAS",  value: String(blocks.length),    color: C.bluestone },
          ...(seriesCount > 0 ? [{ label: "SÉRIES", value: String(seriesCount), color: C.slate }] : []),
        ].map(stat => (
                    ...
                    {isSeries ? (
                      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                        ...
                        <span style={{ fontSize: 13, color: C.muted }}>
                          ...
                          {" "}{fmt(b.workDurationMin ?? 2)} esforço
        ...
        {blocks.length === 0 ? (
          <div style={{
            ...
            <span style={{ fontSize: 12, color: "#333" }}>
              Adicione aquecimento, séries, recuperação e desaquecimento.
            </span>
          </div>
        ) : (
          <>
