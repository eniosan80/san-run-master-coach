      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "20px 20px 0", minHeight: 0 }}>
        ...
        <div style={{ marginBottom: 20 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 10 }}>
            <div>
              <div style={{ fontSize: 13, fontWeight: 700, color: C.offwhite }}>Como foi o esforço?</div>
        ...
        <div style={{ marginBottom: 24 }}>
          <div style={{ fontSize: 11, color: C.muted, textTransform: "uppercase", letterSpacing: 2, marginBottom: 10 }}>
            Observação <span style={{ fontSize: 10, fontWeight: 400, textTransform: "none", letterSpacing: 0 }}>(opcional)</span>
          </div>
          <textarea
...
function BuilderTimer({ steps, workoutName }: { steps: ExecStep[]; workoutName: string }) {
  ...
  function playCountdown(remaining: number) {
    ...
    if (remaining === 0) beep(1100, 160, 0.4); // bipe conclusão
  }

  const advanceStep = useCallback(() => {
    const next = stepIdxRef.current + 1;
    if (next >= steps.length) {
      // VOICE_HOOK: TTS "Treino concluído! Parabéns, atleta SAN RUN."
      beep(880, 100, 0.35, 0);
...
function FreeRunTimer({ title, rpe: suggestedRpe }: { title: string; rpe: number }) {
  ...
  const coach = !started
    ? "Pronto. Aperte INICIAR quando quiser começar."
    : running
      ? "Cada passo faz parte do plano."
...
export default function TimerPage() {
  ...
  // 3. Fallback: cronômetro livre (sem estrutura de blocos)
  return (
    <FreeRunTimer
