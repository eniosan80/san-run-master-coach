...
function getMotivation(streak: number, total: number) {
  if (total === 0) return "Seu primeiro treino começa agora. Vai ser épico.";
  if (streak >= 14) return `${streak} dias consecutivos. Você é elite.`;
  if (streak >= 7)  return `${streak} dias seguidos. Isso é raça de verdade.`;
  if (streak >= 3)  return `${streak} dias consecutivos. Constância em movimento.`;
  if (streak === 1) return "Primeiro passo dado. Amanhã é o segundo.";
  return "Resultado vem do processo. Continue.";
}

const LEVEL_NAMES    = ["","Iniciante Absoluto","Corredor Iniciante","Intermediário","Avançado","Performance"];
    ...
    <div style={{ position:"fixed",inset:0,zIndex:300,background:"rgba(0,0,0,0.72)",backdropFilter:"blur(8px)",display:"flex",alignItems:"flex-end",animation:"fadeInBg 0.2s ease both" }}>
      <div style={{ width:"100%",maxWidth:430,margin:"0 auto",background:"linear-gradient(180deg,rgba(20,20,34,0.99) 0%,#0B0B0F 100%)",borderTop:"1px solid rgba(196,98,45,0.3)",borderRadius:"24px 24px 0 0",padding:"28px 20px 44px",animation:"slideUp 0.35s cubic-bezier(.34,1.4,.64,1) both" }}>
        ...
        <p style={{ textAlign:"center",fontSize:"0.78rem",color:"var(--muted)",marginBottom:26,lineHeight:1.55 }}>
          Cada treino é um ato de comprometimento com quem você quer se tornar.
        </p>
        <div style={{ display:"flex",flexDirection:"column",gap:10 }}>
          <SanBtn label="Começar Treino" onClick={onStartNow}/>
...
export default function DashboardPage() {
  ...
  const lvlName  = LEVEL_NAMES[lvl] || `Nível ${lvl}`;
  ...
  const rpeLabel = (r:number) => r<=3?"Fácil":r<=5?"Moderado":r<=7?"Forte":"Muito forte";
      ...
      <div className="shell screen-with-nav" style={{ overflow:"hidden" }}>
        <div className="screen">
          ...
          <div className="page-content" style={{ position:"relative",zIndex:1 }}>
              ...
              <div style={{ marginTop:14,background:"var(--surface)",border:"1px solid var(--border)",borderRadius:16,padding:"14px 16px" }}>
                <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10 }}>
                  <div style={{ display:"flex",alignItems:"center",gap:8 }}>
                    <div style={{ background:"linear-gradient(135deg,#C4622D,#a04e22)",borderRadius:8,padding:"3px 9px",fontSize:"0.6rem",fontWeight:800,letterSpacing:"0.1em",color:"#fff",textTransform:"uppercase" }}>NÍVEL {lvl}</div>
                ...
                <div style={{ display:"flex",justifyContent:"space-between" }}>
                  {["Inic.","Corr.","Inter.","Avanç.","Perf."].map((l,i)=>(
            ...
            {/* ══ TREINO DE HOJE — só aparece quando hoje é dia de treino no plano semanal ══ */}
            {todayPlanWorkout && (
              <div className="pf d2" style={{ marginBottom:14 }}>
                {todayPlanWorkout.status === "done" ? (
                  <div style={{ background:"var(--surface)",border:"1px solid rgba(196,98,45,0.25)",borderRadius:20,padding:"20px 18px",textAlign:"center" }}>
                    ...
                    <p style={{ fontWeight:800,fontSize:"0.95rem",color:"var(--offwhite)",marginBottom:4 }}>Treino concluído!</p>
                      ...
                      {fp ? (
                        <>
                          ...
                          <p style={{ fontSize:"0.7rem",color:"var(--muted)",lineHeight:1.45 }}>
                            {isForcaToday ? `Hoje é dia de força · ${fp.totalDuration}` : `${fp.blocks.length} exercícios · ${fp.totalDuration}`}
                          </p>
                        </>
                      ) : (
                        <>
                          <p style={{ fontWeight:800,fontSize:"0.88rem",color:"var(--offwhite)",marginBottom:3 }}>FORÇA SAN RUN</p>
              ...
              <div className="pf d2 tap" style={{ background:"var(--surface)",border:"1px solid var(--border)",borderRadius:20,padding:"18px 16px",marginBottom:14,cursor:"pointer" }} onClick={()=>navigate("/weekly")}>
                ...
                <div style={{ marginTop:14 }}>
                  <div style={{ display:"flex",justifyContent:"space-between",marginBottom:6 }}>
                    <span style={{ fontSize:"0.68rem",color:"var(--muted)" }}>{session.weeklyPlan.completedCount}/{session.weeklyPlan.requiredCount} treinos concluídos</span>
                  ...
                  {session.weeklyPlan.completedCount<session.weeklyPlan.requiredCount&&(
                    <p style={{ fontSize:"0.62rem",color:"var(--muted)",marginTop:6,fontStyle:"italic" }}>Próxima semana libera ao completar os treinos planejados.</p>
                  )}
                </div>
              </div>
            )}
            ...
            <div className="pf d4" style={{ background:"var(--surface)",border:"1px solid var(--border)",borderRadius:20,padding:"18px 16px",marginBottom:14 }}>
              <div style={{ display:"flex",alignItems:"center",gap:6,marginBottom:14 }}>
                ...
                <p style={{ fontSize:"0.68rem",fontWeight:800,letterSpacing:"0.1em",textTransform:"uppercase",color:"var(--muted)" }}>Minha Evolução</p>
              </div>
              <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:16 }}>
                <div style={{ background:"rgba(255,255,255,0.04)",borderRadius:14,padding:"12px 8px",textAlign:"center" }}>
                  ...
                  <p style={{ fontSize:"0.56rem",fontWeight:700,letterSpacing:"0.08em",textTransform:"uppercase",color:"var(--muted)" }}>Consistência</p>
                </div>
                <div style={{ background:"rgba(255,255,255,0.04)",borderRadius:14,padding:"12px 8px",textAlign:"center" }}>
                  ...
                  <p style={{ fontSize:"0.56rem",fontWeight:700,letterSpacing:"0.08em",textTransform:"uppercase",color:"var(--muted)" }}>Sequência</p>
                </div>
                <div style={{ background:"rgba(255,255,255,0.04)",borderRadius:14,padding:"12px 8px",textAlign:"center" }}>
              ...
              <p style={{ fontSize:"0.6rem",color:"var(--muted)",fontWeight:600,marginBottom:8 }}>Últimos 7 dias</p>
              ...
              <div className="pf d4" style={{ background:"var(--surface)",border:"1px solid var(--border)",borderRadius:20,padding:"18px 16px",marginBottom:14 }}>
                <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12 }}>
                  <div style={{ display:"flex",alignItems:"center",gap:6 }}>
                    ...
                    <p style={{ fontSize:"0.68rem",fontWeight:800,letterSpacing:"0.1em",textTransform:"uppercase",color:"var(--muted)" }}>Último Check-in</p>
                  </div>
                ...
                <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6 }}>
                  {([{label:"Sono",val:lastCheckin.sleep},{label:"Energia",val:lastCheckin.energy},{label:"Motivação",val:lastCheckin.motivation}] as {label:string;val:number}[]).map(({label,val})=>(
            ...
            <div className="pf d5 tap" style={{ background:"linear-gradient(135deg,rgba(58,95,111,0.12) 0%,var(--surface) 100%)",border:"1px solid rgba(58,95,111,0.3)",borderRadius:20,padding:"18px 16px",marginBottom:14,cursor:"pointer" }} onClick={()=>navigate("/diagnosis")}>
              <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center" }}>
                <div style={{ display:"flex",gap:12,alignItems:"center" }}>
                  ...
                  <div>
                    <p style={{ color:"var(--fg)",fontWeight:700,fontSize:"0.88rem",marginBottom:2 }}>Ver diagnóstico</p>
                    <p style={{ color:"var(--muted)",fontSize:"0.7rem" }}>Análise personalizada do seu perfil</p>
                  </div>
                </div>
            ...
            <button
              ...
              onClick={()=>{ if(confirm("Resetar todos os dados e começar de novo?")){clearSession();navigate("/");} }}
